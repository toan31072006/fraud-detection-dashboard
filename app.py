...
# --- CUSTOM CSS FOR "LIGHT PRO" LOOK ---
st.markdown("""
    <style>
    /* Tổng quan màu nền app (Light Gray/Blue) */
    .stApp {
    ...
    /* Thiết kế lại các KPI Metrics (Thẻ số liệu Nền sáng) */
    div[data-testid="metric-container"] {
        background-color: #FFFFFF;
        border: 1px solid #E2E8F0;
        border-top: 4px solid #2563EB; /* Viền nhấn màu Xanh dương chuẩn Pro */
        padding: 15px 20px;
        border-radius: 12px;
...
# --- TẠO HÀM CHUẨN HÓA GIAO DIỆN BIỂU ĐỒ PLOTLY (LIGHT MODE) ---
def apply_pro_theme(fig):
    """Đồng bộ toàn bộ biểu đồ sang nền sáng, text đen xám"""
    fig.update_layout(
    ...
    fig.update_yaxes(gridcolor="#E2E8F0", zeroline=False, linecolor="#E2E8F0")
    return fig
...
if not df.empty:
    ...
    if page == "Industry Overview":
        ...
        with r1_c1:
            ...
            fig1 = px.line(yearly_stats, x='Year', y='fraud_score', markers=True, title="Average Industry Fraud Score Trend", color_discrete_sequence=['#EF4444'])
    ...
    # ====================================================
        ...
        with r3_c1:
            ...
            fig6 = px.bar(flag_sums, x='Count', y='Red Flag Type', orientation='h', title="Most Common Fraud Types", color='Count', color_continuous_scale='Reds')
    ...
    # ====================================================
        ...
        with r11_c1:
            ...
            fig24 = px.treemap(top_rev, path=[px.Constant("Market"), 'Symbol'], values='Rev_Plot', color='fraud_score', color_continuous_scale='Reds', title="Top 50 Companies Heatmap (Size = Rev, Color = Risk)")
        ...
        with r12_c1:
            ...
            fig26 = go.Figure(data=[
                ...
                go.Bar(name='Long-Term Debt', x=cap_struct['Year'], y=cap_struct['LT_Debt'], marker_color='#EF4444')
            ])
